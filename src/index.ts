...
function setPropName(filterKey: string, propPrefix: string) {
  if (propPrefix) {
    const isRoot = filterKey === ITEM_ROOT
    const isIndexOfRegex = INDEXOF_REGEX.test(filterKey);
    const isFunctionRegex = FUNCTION_REGEX.test(filterKey);
    ...
    if (isIndexOfRegex) {
      return changeFilterKey(filterKey, propPrefix, INDEXOF_REGEX);
    }

    if (isFunctionRegex) {
      return changeFilterKey(filterKey, propPrefix, FUNCTION_REGEX);
    }

    return `${propPrefix}/${filterKey}`;
  }

  return filterKey;
}

function checkIsLogicalValue(value: string) {
  return LOGICAL_OPERATORS.indexOf(value) !== -1
}
...
function changeFilterKey(filterKey: string, propPrefix: string, regex: RegExp) {
  return filterKey.replace(
    regex, (_, $1) => $1.trim() === ITEM_ROOT
      ? `(${propPrefix})`
      : `(${propPrefix}/${$1.trim()})`
  );
}

function handlePrimitiveValue(key: string, val: any, aliases: Alias[] = []) {
...
function handleArrayValue({
  ...
  const result = []
  const op = key;
  const isLogicalOperator = checkIsLogicalValue(op)

  const builtFilters = value
    .map(v => buildFilter(v, aliases, propPrefix))
    .filter(f => f)
    .map(f => (isLogicalOperator ? `(${f})` : f));
...
function handleLogicalValue({
  ...
  const result = []
  const op = propName;
  const builtFilters = Object
    .keys(value)
    .map(valueKey => buildFilterCore({ [valueKey]: value[valueKey] }));
...
function handleObjectValue({
  ...
  if ('type' in value) {
    result.push(handlePrimitiveValue(propName, value, aliases));
  } else {
    ...
    operators.forEach(op => {
      ...
      if (COMPARISON_OPERATORS.indexOf(op) !== -1) {
        ...
        if (Array.isArray(value[op])) {
          result.push(
            value[op]
              .map((v: any) => '(' + buildFilter(v, aliases, propName) + ')')
              .join(` ${op} `)
          );
        } else {
          result.push('(' + buildFilter(value[op], aliases, propName) + ')');
        }
      } else if (COLLECTION_OPERATORS.indexOf(op) !== -1) {
        const collectionClause = buildCollectionClause({ lambdaParameter: key.toLowerCase(), value: value[op], op, propName, aliases });
        ...
        // Nested property
        const doesPropNameContainNot = /\/not/g.test(propName)
        let internalPropName = doesPropNameContainNot ? propName.replace(/\/not/g, '') : propName

        const filter = doesPropNameContainNot
          ? parseNot([buildFilter({ [op]: value[op] }, aliases, internalPropName)])
          : buildFilter({ [op]: value[op] }, aliases, internalPropName)
  ...
  if (typeof filter === 'string') {
    ...
    const filtersArray = Object.keys(filter).reduce(
      (result: any[], filterKey) => {
        ...
        const isRoot = filterKey === ITEM_ROOT
        const propName = setPropName(filterKey, propPrefix);

        if (isRoot && Array.isArray(value)) {
          return result.concat(
            value.map((arrayValue: any) => handlePrimitiveValue(propName, arrayValue))
          )
        }

        const isPrimitiveValue = ['number', 'string', 'boolean'].indexOf(typeof value) !== -1 ||
        ...
        if (isPrimitiveValue) {
          ...
          result.push(...handleArrayValue({ key: filterKey, value, aliases, propPrefix }))
        } else if (checkIsLogicalValue(propName)) {
          result.push(...handleLogicalValue({ propName, value }))
        } else if (isObjectValue) {
          result.push(...handleObjectValue({ key: filterKey, propName, value, aliases }))
        } else {
...
function buildCollectionClause({
  ...
  if (typeof value === 'string' || value instanceof String) {
    ...
    const filter = buildFilter(filterValue, aliases, lambdaParameter);
